...
带源码的 control repo、外部数据盘和 runtime/third-party 分层模板见 `workspace-layout.md`；不要把
推荐树机械套进作者仓。
...
| `references/` | 稳定文献、代码谱系、研读快照、调研综合和 ingestion provenance | 来源核验或接收稳定快照时 | 运行状态、未核搜索线索和可变 inbox |
| `discussion/` | 子 agent、红队和未收敛草稿 | 按需 | 正典 claim 和唯一状态 |

协作者交付目录是 staging inbox，不是信息真源。收纳时复制点名文件到 `references/` 或相应
`discussion/<author>/`，记录 source-to-destination hash，并保留 inbox；具体生命周期见
...
Experiment 是可证伪问题，不等于一次进程。在正式 run registry 落地前，run tag 必须以
`EXP-####` 开头，并包含任务、seed、预算单位和 UTC 时间；详细命名见 `git-and-naming.md`。
...
- status 发现 orphan experiment 时先对账：可恢复的卡显式 `reconcile --adopt`；只有 schema v1
  且已有卡片/文档证据的结果才允许 `--complete-legacy`；现代实验必须走正式 close。
...
实验可附带 `cycle_class=formal|replication|probe|oracle|instrumentation`。后三类诊断模板自动补全通用控制项，证据
...
- 明确 control/runtime/workflow/third-party/artifact-store 五类角色，各自记录 remote、branch、commit 和 dirty state。
- 不使用 `git add .` 收纳研究目录；精确提交进入当前证据链的文件，不删除不明来源 untracked artifacts。
- formal/replication 必须从 tracked-clean source state 冻结；诊断 dirty patch 必须保存完整 diff/hash，且证据权限不升级。
- freeze 后不得 rebase、squash 或 force-push 被引用历史；结案后保留 frozen commit 可达。
- dump -> offline analysis -> online validation 尽量在同一 tracked commit 完成。
- runtime patch 默认关闭并有独立配置入口；行为保持重构和算法改变分开提交。
- branch、commit、tag、文件、变量单位和历史迁移规则见 `git-and-naming.md`。
